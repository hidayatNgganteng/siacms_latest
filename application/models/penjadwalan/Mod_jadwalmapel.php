...
class Mod_jadwalmapel extends CI_Model {
	...
	public function getjadwalmapel($where = array()){
		...
		$this->db->join('hari_rentang','jadwal_mapel.jam_ke = hari_rentang.jam_ke AND jadwal_mapel.hari = hari_rentang.hari 
		AND jadwal_mapel.id_tahun_ajaran = hari_rentang.id_tahun_ajaran', 'left');
		$this->db->where($where);
		return $this->db->get('jadwal_mapel')->result();	
	}
