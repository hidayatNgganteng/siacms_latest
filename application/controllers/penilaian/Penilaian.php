...
class Penilaian extends CI_Controller
{
    ...
    public function nilaisiswa()
    {
        ...
        $mapel = $this->M_data->getMatapelajaran(
            array('kelas_reguler.id_kelas_reguler' => $id_kelas_reguler_berjalan, 'kelas_reguler_berjalan.id_tahun_ajaran' => $id_tahun_ajaran,
            ));

        if ($id_mapel == "") {
            $id_mapel = @$data['mapel'][0]->id_mapel;
        }

        //print_r(json_encode($data['kelas_reguler_berjalan']));die();
        ...
        foreach ($siswaperkelas as $rowsiswa) {
            foreach ($mapel as $rowmapel) {
                ...
                foreach ($kategorinilai as $rowkategorinilai) {
                    $np = $np+@$this->M_data->getNilaiSiswa([
                        'nilai_siswa.nisn' => $rowsiswa->nisn,
                        'nilai_siswa.mapel_id' => $rowmapel->id_mapel,
                        'jenis_nilai_akhir.kode_na' => '1',
                        'id_kategorinilai' => $rowkategorinilai->id_kategorinilai,
                    ])->nilai;
                    $nk = $nk+@$this->M_data->getNilaiSiswa([
                        'nilai_siswa.nisn' => $rowsiswa->nisn,
                        'nilai_siswa.mapel_id' => $rowmapel->id_mapel,
                        'jenis_nilai_akhir.kode_na' => '2',
                        'id_kategorinilai' => $rowkategorinilai->id_kategorinilai,
                    ])->nilai;
                }
    ...
    {
        $this->load->model('penilaian/M_data');
        $data['f'] = $this->M_data->selectNilai($id);
    ...
    public function importnilai()
    {
        ...
        if (isset($_FILES['filenilai']['name']) && in_array($_FILES['filenilai']['type'], $file_mimes)) {

            $arr_file = explode('.', $_FILES['filenilai']['name']);
            ...
            for ($i = 1; $i < count($sheetData); $i++) {
                if ($read) {
                    ...
                    $nisn = $sheetData[$i][1];
                    $nilai = $sheetData[$i][3];
                    if ($nisn !== null && !empty($nisn) && $nilai !== null && !empty($nilai)) {
                        $arrdata = array(
                            'nisn' => $nisn,
                        ...
                        $datnilai = $this->M_data->cekNilai($nisn, $id_mapel, $id_kategorinilai, $id_jenis_na);
                        if ($datnilai) {
                ...
                if (strcasecmp($sheetData[$i][0], 'no') == 0 && strcasecmp($sheetData[$i][1], 'nisn') == 0) {
                    $read = true;
                }
            }
